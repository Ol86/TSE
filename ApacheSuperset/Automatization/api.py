...
def updateDashboard(url, csrf_token, auth_token, session):
    headers = {
        'Authorization': f'Bearer {auth_token}',
        'X-CSRFToken': csrf_token,
    }
    ...
    ids_to_update = get_charts_from_dashboard(getCharts(url, auth_token),
                                              get_dashboard_id(getDashboards(url, auth_token),
                                                               'Standarized Dashboard'))  # TODO here the dashboard id dynamically

    body = {
        "certification_details": "string",
        "certified_by": "ale",
        "css": "string",
        "dashboard_title": "Standarized Dashboard",
        "external_url": "string",
        "is_managed_externally": True,
        "json_metadata": "",
        "owners": [1],
    ...
    dashboard_res = session.put(
        f'{url}/api/v1/dashboard/' + str(get_dashboard_id(getDashboards(url, auth_token), 'Standarized Dashboard')),
...
## ----------------------------------------------------------------
def createStandarizedDashboard(url, csrf_token, auth_token, session):
    headers = {
        'Authorization': f'Bearer {auth_token}',
        'X-CSRFToken': csrf_token,
    }
    ...
    createChart_numberOfSessions(url, csrf_token, auth_token, session)
    createChart_experimentInformation(url, csrf_token, auth_token, session)
    createChart_questions(url, csrf_token, auth_token, session)
    createChart_numberOfWatches(url, csrf_token, auth_token, session)
...
def createChart_numberOfSessions(url, csrf_token, auth_token, session):
    headers = {
        'Authorization': f'Bearer {auth_token}',
        'X-CSRFToken': csrf_token,
    }
    ...
    body = {
        ...
        "dashboards": [
            ...
            # TODO maybe need to count the number of Dashboards
        ],
        "datasource_id": get_datasource_id(getDatasets(url, auth_token), "base_session"),
        "datasource_name": "base_session",  # TODO here user specific dataset
        "datasource_type": "table",
        "description": None,
        "is_managed_externally": False,
        "owners": [
            1  # TODO here define the owner (user)
        ],
...
def createChart_numberOfWatches(url, csrf_token, auth_token, session):
    headers = {
        'Authorization': f'Bearer {auth_token}',
        'X-CSRFToken': csrf_token,
    }
    ...
    body = {
        ...
        "dashboards": [
            get_dashboard_id(getDashboards(url, auth_token), "Standarized Dashboard")],
        "datasource_id": get_datasource_id(getDatasets(url, auth_token), "base_experiment_watch_id"),
        "datasource_name": "base_experiment_watch_id",  # TODO here user specific dataset
        "datasource_type": "table",
        "description": None,
        "is_managed_externally": False,
        "owners": [
...
def createChart_experimentInformation(url, csrf_token, auth_token, session):
    headers = {
        'Authorization': f'Bearer {auth_token}',
        'X-CSRFToken': csrf_token,
    }
    ...
    body = {
        ...
        "dashboards": [
            get_dashboard_id(getDashboards(url, auth_token), "Standarized Dashboard")],
        "datasource_id": get_datasource_id(getDatasets(url, auth_token), "base_experiment"),
        # TODO maybe need to change here
        "datasource_name": "base_experiment",
        "datasource_type": "table",
        "description": None,
        "is_managed_externally": False,
        "owners": [
...
def createChart_questions(url, csrf_token, auth_token, session):
    headers = {
        'Authorization': f'Bearer {auth_token}',
        'X-CSRFToken': csrf_token,
    }
    ...
    body = {
        ...
        "dashboards": [
            get_dashboard_id(getDashboards(url, auth_token), "Standarized Dashboard")],
        "datasource_id": get_datasource_id(getDatasets(url, auth_token), "base_questions"),
        # TODO maybe need to change here
        "datasource_name": "base_questions",
        "datasource_type": "table",
        "description": None,
        "is_managed_externally": False,
        "owners": [
...
def createChart_hr(url, csrf_token, auth_token, session):
    headers = {
        'Authorization': f'Bearer {auth_token}',
        'X-CSRFToken': csrf_token,
    }
...
def createDataset_answers(url, csrf_token, auth_token, session):
    headers = {
        'Authorization': f'Bearer {auth_token}',
        'X-CSRFToken': csrf_token,
    }
    ...
    body = {
        ...
        "normalize_columns": False,
        "owners": [
            1      #TODO user definition for the dataset
        ],
        "schema": "public",
        "sql": "SELECT base_questionanswers.id AS answer_id, answer, question_id, experiment_id, session_id\nFROM base_questionanswers \nJOIN base_answers ON base_questionanswers.id = base_answers.answer_id\n",
        "table_name": "base_answers_ids"
    }
    dataset_resp = session.post(f'{url}/api/v1/dataset/', headers=headers, json=body)
    dataset = dataset_resp.json()
    saveToFile(json.dumps(dataset, indent=4))
...
def createChart_answers(url, csrf_token, auth_token, session):
    headers = {
        'Authorization': f'Bearer {auth_token}',
        'X-CSRFToken': csrf_token,
    }
    ...
    body = {
        ...
        "datasource_id": get_datasource_id(getDatasets(url, auth_token), "base_answers_ids"),
        "datasource_name": "base_answers_ids",
        "datasource_type": "table",
        "description": None,
        "is_managed_externally": False,
        "owners": [
        ...
        "params": "{\"datasource\":\"18__table\",\"viz_type\":\"pie\",\"groupby\":[\"answer\"],\"metric\":{\"expressionType\":\"SIMPLE\",\"column\":{\"advanced_data_type\":null,\"certification_details\":null,\"certified_by\":null,\"column_name\":\"answer_id\",\"description\":null,\"expression\":null,\"filterable\":true,\"groupby\":true,\"id\":95,\"is_certified\":false,\"is_dttm\":false,\"python_date_format\":null,\"type\":\"INTEGER\",\"type_generic\":0,\"verbose_name\":null,\"warning_markdown\":null},\"aggregate\":\"COUNT\",\"sqlExpression\":null,\"datasourceWarning\":false,\"hasCustomLabel\":false,\"label\":\"COUNT(answer_id)\",\"optionName\":\"metric_rlqcpobjpal_difuqnkvt27\"},\"adhoc_filters\":[],\"row_limit\":100,\"sort_by_metric\":true,\"color_scheme\":\"supersetColors\",\"show_labels_threshold\":5,\"show_legend\":true,\"legendType\":\"scroll\",\"legendOrientation\":\"top\",\"label_type\":\"key\",\"number_format\":\"SMART_NUMBER\",\"date_format\":\"smart_date\",\"show_labels\":true,\"labels_outside\":true,\"outerRadius\":70,\"innerRadius\":30,\"extra_form_data\":{},\"dashboards\":[]}",
        "query_context": None,
        "query_context_generation": True,
        "slice_name": "Answers",
        "viz_type": "pie"
